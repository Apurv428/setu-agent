...
2. **A shipped side project** — public on GitHub, live at
   [setu-agent.onrender.com](https://setu-agent.onrender.com).
...
8. **Web interface + cloud deployment** — a Streamlit app deployed on Render exposes
   the full agent (text input, mic recording, tool trace, synthesized audio) without
   any local setup.
...
```
  User (browser or terminal)
        |
        |-- streamlit_app.py  (web UI — Render / localhost:8501)
        |     text input + st.audio_input mic widget
        |
        |-- app.py  (CLI — mic, --text, --chat REPL, --demo)
        |
        v  audio path / text query
...
it is rebuilt. This keeps startup fast after the first run. On Render the filesystem is
ephemeral, so the cache rebuilds on each cold start (~10 seconds added to first request).
...
on the next call. The caller owns the history object.
...
### 3.5 `app.py` — the CLI voice entrypoint

A thin CLI with four modes:
...
### 3.6 `streamlit_app.py` — the web interface

A Streamlit app that wraps the scratch agent with a browser UI. It is deployed at
[setu-agent.onrender.com](https://setu-agent.onrender.com) and auto-redeploys on
every push to `main`.

**Input modes:**
- **Text tab** — form input, submits on Enter or the Send button.
- **Voice tab** — `st.audio_input` mic widget. When the user stops recording,
  the audio bytes are saved to a temp WAV file and passed to the agent as a file
  path (same path the CLI takes). Duplicate submissions are deduplicated by hashing
  the audio bytes.

**Per-turn output:**
- `st.chat_message` bubbles for user and assistant.
- A collapsible **Tool calls** expander with the raw stdout from the agent loop
  (captured via `contextlib.redirect_stdout`).
- `st.audio` player for the Bulbul v3 synthesized WAV, read from disk into bytes.

**State:**
- `st.session_state.history` — the scratch agent's accumulated message list, kept
  across browser requests within a session.
- `st.session_state.chat` — the display-side chat log (role, content, steps, audio).
- `st.cache_resource` — loads the sentence-transformers model once per server
  process; all sessions share the cached model.

**Deployment** via `render.yaml`: Python web service, `pip install -r requirements.txt`
build, `streamlit run streamlit_app.py --server.port $PORT --server.address 0.0.0.0
--server.headless true` start command, `SARVAM_API_KEY` set as an environment secret.

---

## 4. Data flow of one turn (the agent's decision, not a fixed pipeline)

**Web turn (streamlit_app.py):**

1. User types text or records voice in the browser.
2. `streamlit_app.py` passes the text string or a temp WAV path to `scratch_agent.run()`.
3. The agent loop runs exactly as in the CLI case below.
4. The final answer text is displayed in a chat bubble; the synthesized WAV plays inline.
...
The sequencing is always the agent's call. Different questions produce different tool
sequences — that is the whole point.
...
`"judge_parse_error"` recorded. API errors are also counted as FAIL — the runner never
crashes mid-run.

**`qa` cases route through the scratch agent** so the eval measures the full RAG +
reasoning path, not just the raw model.
...
- `streamlit >= 1.31` — web interface with `st.audio_input` mic widget
- `sounddevice` + `scipy` — audio recording and playback (CLI only)
- `python-dotenv` — config from `.env`
- `pytest` — test suite
- Render — cloud hosting (`render.yaml`, auto-deploy from GitHub)
...
inspector, the Streamlit app, and future clients — tested in isolation before any agent
touches it.
...
**Why Streamlit for the web interface.** Minimal code on top of the existing agent
— the scratch agent's `run()` function is called unchanged. `st.audio_input` gives a
mic widget with no JavaScript. `st.cache_resource` keeps the embedding model warm
across requests. Render auto-deploys from GitHub, so every commit to `main` is live
within minutes.
...
- **Ephemeral filesystem on Render.** The `.index.npz` cache is lost on each cold
  start and rebuilt on the first request (~10s extra). The knowledge docs are always
  present (checked into the repo).
...
- **Persistent RAG cache on Render** — mounting a Render disk would keep `.index.npz`
  across restarts and eliminate the cold-start rebuild cost.
